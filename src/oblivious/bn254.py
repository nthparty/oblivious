...
global scl, rnd, inv, smu, sad, ssb, sne, pnt, bas, mul, add, sub, neg, par, ser, des, sse, sde,\
       nrm, rnd2, scl2, inv2, smu2, sad2, pnt2, bas2, mul2, add2, sub2, neg2, des2, sde2
scl = native.scl
rnd = native.rnd
inv = native.inv
smu = native.smu
...
global point, scalar, point2, scalar2

backends = (native, mcl) if mclbn256 else (native)
for lib in backends:
    global point, scalar, point2, scalar2

    # Encapsulate pure-Python and MClBN backend functions with their top-level best-effort synonyms.
    scl = lib.scl
    rnd = lib.rnd
    inv = lib.inv
    smu = lib.smu
    ...
    class point(G1): # pylint: disable=W0621,E0102
        ...
        def __matmul__(self: point, other: point2) -> scalar2:
            ...
            >>> z_mcl    = 'd01f7e038b05acc5519eeda026c4aa111eb12f3483f274c60e34e6ec7571435df707'
            >>> z_native = '731ff16849a86c40280717696a8aa44fbe16f565f087d003413d141de7f5d109fc0c'
            >>> mclbn256 = p.__class__ != lib.point # (In case both native and mcl are defined.)
            >>> z == z_mcl if mclbn256 else z == z_native
            True
        ...
        def normalize(self: point) -> point:
            ...
            True
            >>> p.to_bytes() == p.normalize().to_bytes() and p.__class__ != lib.point
            False
    ...
    class scalar(Fr): # pylint: disable=E0102
        ...
        def __int__(self: scalar) -> bytes:
            ...
            n = (int.from_bytes(sse(self), 'little') * d_inv) % r
        ...
        def to_int(self: scalar) -> bytes:
            ...
            """
            d_inv = 0x235f846d22752e25720e909a9e82a1b4ad47e882341d8fca46c142d23fa9bc45
            return (int.from_bytes(sse(self), 'little') * d_inv) % r
    ...
    class point2(G2): # pylint: disable=W0621,E0102
        ...
        def normalize(self: point2) -> point2:
            ...
            True
            >>> q.to_bytes() == q.normalize().to_bytes() and q.__class__ != lib.point2
            False
    ...
    class scalar2(GT): # pylint: disable=function-redefined
        ...
        def hash(cls, bs: Union[bytes, bytearray]) -> scalar2:
            ...
            >>> mclbn256 = s.__class__ != lib.scalar2 # (In case both native and mcl are defined.)
            >>> z == z_mcl if mclbn256 else z == z_native
            True
            """
